...
describe("DetalleTablero", () => {
  ...
  it("does not render the BOM panel in DetalleTablero", async () => {
    vi.stubGlobal(
      "fetch",
    ...
    expect(screen.queryByText(/LISTA DE MATERIALES \(BOM\)/i)).not.toBeInTheDocument();
  });
  ...
  it("does not leak form values between secciones when switching tabs", async () => {
    ...
    const cargaInputSeccion1 = screen.getAllByLabelText(/carga/i)[0] as HTMLSelectElement;
    await userEvent.selectOptions(cargaInputSeccion1, "16");
  ...
  it("closes the nivel de falla modal with Escape directly without saving", async () => {
    renderDetalle();
    await screen.findByRole("tab", { name: "Sección 1" });
  ...
  it("closes the nivel de falla modal by clicking the backdrop directly without saving", async () => {
    renderDetalle();
    await screen.findByRole("tab", { name: "Sección 1" });
  ...
  it("closes the Icc modal directly when closing via Cancelar without confirmation", async () => {
    renderDetalle();
    await screen.findByRole("tab", { name: "Principal" });
